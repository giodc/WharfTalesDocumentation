...
<head>
    ...
    <title><?php echo htmlspecialchars($title); ?> -
        <?php echo htmlspecialchars($GLOBALS['config']['site_name'] ?? 'Documentation'); ?></title>
...
<body class="antialiased">
        ...
        <div class="container-fluid mx-auto px-2 py-4 flex items-center justify-between">
            <div class="flex items-center space-x-2">
                ...
                <?php if (!empty($GLOBALS['config']['versions']['enabled']) && !empty($GLOBALS['config']['versions']['show_selector']) && !empty($versions)): ?>
                    <div class="relative ml-4">
                        <button id="version-selector"
                            class="px-3 py-1.5 text-sm border rounded-lg flex items-center space-x-2 hover:bg-gray-50 dark:hover:bg-gray-800"
                            style="border-color: var(--color-border);">
                            <span><?php echo htmlspecialchars($currentVersion ?? 'Select Version'); ?></span>
                            <?php if (isset($versionManager)): ?>
                                <span class="text-xs"
                                    style="color: var(--color-text-secondary);"><?php echo $versionManager->getVersionBadge($currentVersion); ?></span>
                            <?php endif; ?>
                        ...
                        <div id="version-dropdown"
                            class="absolute top-full mt-2 left-0 w-48 rounded-lg shadow-lg hidden z-50"
                                ...
                                <a href="/<?php echo htmlspecialchars($versionPath); ?>"
                                    ...
                                    <?php if (!empty($version['status']) && $version['status'] !== 'stable'): ?>
                                        <span class="text-xs ml-1"
                                            style="color: var(--color-text-secondary);">(<?php echo htmlspecialchars(ucfirst($version['status'])); ?>)</span>
                                    <?php endif; ?>
            ...
            <div class="flex items-center space-x-2">
                ...
                <div class="relative hidden md:block">
                    ...
                    <div id="search-results" class="absolute top-full mt-2 w-full rounded-lg shadow-lg hidden"
                        style="background-color: var(--color-bg); border: 1px solid var(--color-border); max-height: 400px; overflow-y: auto;">
                    </div>
                </div>
    ...
    <div class="md:hidden border-b px-4 py-3"
        style="background-color: var(--color-bg); border-color: var(--color-border);">
        <div class="relative">
            <input type="text" id="search-input-mobile" placeholder="Search documentation..."
                class="w-full px-4 py-2 rounded-lg border"
                style="background-color: var(--color-bg-secondary); border-color: var(--color-border);">
            <div id="search-results-mobile" class="absolute top-full mt-2 w-full rounded-lg shadow-lg hidden"
                style="background-color: var(--color-bg); border: 1px solid var(--color-border); max-height: 400px; overflow-y: auto; z-index: 40;">
            </div>
        </div>
    </div>
    ...
    <div class="flex">
        <!-- Sidebar -->
        <aside id="sidebar"
            class="sidebar w-64 lg:w-64 h-screen overflow-y-auto hidden lg:block fixed lg:sticky lg:top-16 top-0 lg:relative z-40">
            <nav class="p-4">
                ...
                <?php if (!empty($GLOBALS['config']['social_links_sidebar'])): ?>
                    <div class="mt-8 pt-4 border-t" style="border-color: var(--color-border);">
                        <h4 class="text-xs font-semibold uppercase mb-3" style="color: var(--color-text-secondary);">Connect
                        </h4>
            ...
            <div class="content-wrapper mx-auto max-w-full">
                ...
                <?php if (!empty($editUrl) && $editUrl !== '#'): ?>
                    <div class="mt-8 pt-8 border-t" style="border-color: var(--color-border);">
                        <a href="<?php echo htmlspecialchars($editUrl); ?>" class="text-sm"
        ...
        <?php if (!empty($toc)): ?>
            <aside class="hidden xl:block w-64 h-screen sticky top-20 overflow-y-auto p-4">
                <h3 class="font-semibold mb-4 text-sm uppercase" style="color: var(--color-text-secondary);">On This Page
                </h3>
                <nav class="space-y-2">
                    <?php foreach ($toc as $item): ?>
                        <a href="#<?php echo htmlspecialchars($item['slug']); ?>" class="block text-sm hover:text-zinc-600"
        ...
        <div class="container-fluid mx-auto px-4 py-6">
            <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div class="text-sm" style="color: var(--color-text-secondary);">
                    ...
                    <?php else: ?>
                        Powered by <a href="https://github.com/wharftales/wharfdocs" target="_blank"
                            rel="noopener noreferrer" style="color: var(--color-primary);"
                            class="hover:underline">WharfDocs</a>
                    <?php endif; ?>
                </div>
                                ...
                                searchResults.innerHTML = results.map(result => `
                                    <div class="search-result" onclick="window.location.href='/${result.path}'">
                                        <div class="flex items-center justify-between">
                                            ...
                                            ${result.version ? `<span class="text-xs px-2 py-0.5 rounded" style="background-color: var(--color-primary); color: white;">${result.version}</span>` : ''}
                                        </div>
